...
mkPat' args orig (Ref fc (DataCon ref t a) n) = PCon fc n t a args
...
mkTerm vars (PCon fc x tag arity xs)
    -- Should we get the reference to the scrutinee here?
    --                            v
    = apply fc (Ref fc (DataCon Nothing tag arity) x)
               (map (mkTerm vars) xs)
