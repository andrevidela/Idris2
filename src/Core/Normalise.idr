  ...
  mutual
    ...
    evalLocClosure {free} env fc mrig stk (MkNFClosure nf)
      ...
      where
        ...
        applyToStack (NDCon fc ref n t a args) stk
            = pure $ NDCon fc ref n t a (args ++ stk)
    ...
    evalRef : {free : _} ->
              ...
              Core (NF free)
    evalRef env meta fc (DataCon ref tag arity) fn stk def
        = pure $ NDCon fc ref fn tag arity stk
    ...
    tryAlt {more} env loc opts fc stk (NDCon _ ref nm tag' arity args') (ConCase x tag args sc)
         = if tag == tag'
...
mutual
  ...
  Convert NF where
    ...
    convGen q defs env (NDCon _ _ nm tag _ args) (NDCon _ _ nm' tag' _ args')
        = if tag == tag'
             then allConv q defs env args args'
             else pure False
    ...
    repSub (NApp fc hd args)
        = do args' <- traverse repArg args
             pure $ apply fc
                        ...
                        args'
    repSub (NDCon fc ref n t a args)
        = do args' <- traverse repArg args
             empty <- clearDefs defs
             pure $ apply fc
                        !(quote empty env (NDCon fc ref n t a []))
                        args'
