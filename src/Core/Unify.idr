...
mutual
  ...
  unifyHoleApp swap mode loc env mname mref margs margs' (NDCon nfc ref n t a args')
      = do defs <- get Ctxt
           mty <- lookupTyExact n (gamma defs)
           unifyInvertible swap (lower mode) loc env mname mref margs margs' mty (NTCon nfc n t a) args'
  ...
  unifyNoEta : {auto c : Ref Ctxt Defs} ->
               ...
               NF vars -> NF vars ->
               Core UnifyResult
  unifyNoEta mode loc env (NDCon xfc xref x tagx ax xs) (NDCon yfc yref y tagy ay ys)
      = do gam <- get Ctxt
           if tagx == tagy
             then
                  do ust <- get UST
                     ...
                     unifyArgs mode loc env xs ys
                       ...
                       (NDCon xfc xref x tagx ax xs)
                       (NDCon yfc yref y tagy ay ys)
