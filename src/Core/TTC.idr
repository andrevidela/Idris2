...
TTC NameType where
  ...
  fromBuf b
      = case !getTag of
             0 => pure Bound
             1 => pure Func
             2 => do rig <- fromBuf b; x <- fromBuf b; y <- fromBuf b; pure (DataCon rig x y)
...
TTC Def where
  ...
  toBuf b (DCon rig t arity nt) = do tag 4; toBuf b rig; toBuf b t; toBuf b arity; toBuf b nt
  ...
  fromBuf b
      = case !getTag of
             ...
             4 => do rig <- fromBuf b; t <- fromBuf b; a <- fromBuf b; nt <- fromBuf b
                     pure (DCon rig t a nt)
