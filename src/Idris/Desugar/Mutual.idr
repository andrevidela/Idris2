...
getDecl AsType (PData fc doc header (MkPData dfc tyn (Just tyc) _ _))
    = Just (PData fc doc header (MkPLater dfc tyn tyc))
getDecl AsType d@(PInterface _ _ _ _ _ _ _ _ _) = Just d
getDecl AsType (PRecord fc doc header (MkPRecord n ps _ _ _))
    = Just (PData fc doc header (MkPLater fc n (mkRecType ps)))
  where
...
getDecl AsDef d@(PRecord _ _ _ (MkPRecordLater _ _)) = Just d
