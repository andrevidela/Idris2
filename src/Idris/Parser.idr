...
mutual
  ...
  pibindListName fname indents
       = do rig <- multiplicity fname
            ...
            ty <- typeExpr pdef fname indents
            atEnd indents
            pure (map (\n => MkBasicBinder rig n.withFC ty) ns)
    where
...
implBinds fname indents namedImpl = concatMap (map adjust) <$> go where
  ...
  go = do decoratedSymbol fname "{"
          ...
          ns <- map @{Compose} (\(MkBasicBinder rc wb n) => (rc, wb, piInfo.val, n)) $ forget <$> pibindListName fname indents
          commitSymbol fname "}"
          commitSymbol fname "->"
          more <- go
          pure (ns :: more)
...
paramDecls fname indents = do
         ...
         commit
         args <- bounds (Right <$> newParamDecls fname indents
