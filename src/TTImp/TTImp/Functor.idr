...
mutual
  ...
  Functor RawImp' where
    ...
    map f (IBindingApp fc binder pat bound body)
      = IBindingApp fc binder (map f pat) (map f bound) (map f body)
  ...
  Functor ImpDecl' where
    ...
    map f (IData fc hd dt)
      = IData fc hd (map f dt)
    ...
    map f (IRecord fc cs header rec)
      = IRecord fc cs header (map f rec)
